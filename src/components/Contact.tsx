...
/* ============================================================
   ...
   S'ouvre aussi depuis le header (event "openContactModal" ou ?contact=1)
   ============================================================ */
...
export default function Contact() {
  ...
  useEffect(() => {
    const openIt = () => { setSent(false); setOpenModal(true); };
    window.addEventListener("openContactModal", openIt);
    if (typeof window !== "undefined" && new URLSearchParams(window.location.search).get("contact") === "1") openIt();
    const onKey = (e: KeyboardEvent) => e.key === "Escape" && setOpenModal(false);
    document.addEventListener("keydown", onKey);
    return () => { window.removeEventListener("openContactModal", openIt); document.removeEventListener("keydown", onKey); };
  }, []);

  useEffect(() => { document.body.style.overflow = openModal ? "hidden" : ""; return () => { document.body.style.overflow = ""; }; }, [openModal]);
  ...
  const fieldStyle: React.CSSProperties = {
    ...
    borderRadius: 0, padding: "14px 16px", color: WHITE, fontSize: "14px", outline: "none", fontFamily: "inherit",
  };
  ...
  return (
    <>
      {/* ===== BANDE ===== */}
      <section id="contact"
        style={{ background: INK, padding: "clamp(80px, 12vh, 150px) 24px", textAlign: "center",
        ...
        <button onClick={() => { setSent(false); setOpenModal(true); }}
          style={{ display: "inline-flex", alignItems: "center", gap: "14px", background: "transparent",
            border: `1px solid ${GOLD}`, color: WHITE, fontSize: "11px", letterSpacing: "0.3em",
            textTransform: "uppercase", fontWeight: 500, padding: "20px 46px", cursor: "pointer", transition: "all .35s ease" }}
      ...
      {/* ===== POP-UP ===== */}
      {openModal && (
        <div onClick={(e) => { if (e.target === e.currentTarget) setOpenModal(false); }}
          ...
          <div style={{ position: "relative", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto",
            ...
            {sent ? (
              ...
              <>
                ...
                <h3 style={{ color: WHITE, fontWeight: 300, fontSize: "1.7rem", letterSpacing: "-0.01em", marginBottom: "26px" }}>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div style={{ marginBottom: "24px" }}>
                    ...
                    <textarea rows={4} placeholder="Tell me about your project..." value={form.message}
                      onChange={(e) => setForm({ ...form, message: e.target.value })} style={{ ...fieldStyle, resize: "none" }} />
                  </div>
                  <button type="submit" disabled={sending}
                    style={{ width: "100%", padding: "16px", background: WHITE, border: `1px solid ${WHITE}`, color: INK,
